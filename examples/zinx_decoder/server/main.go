...
func main() {
	...
	s.AddRouter(0x10, &s_router.HelloZinxRouter{})

	//HTLV+CRC 断粘包
	s.AddInterceptor(zcode.NewLengthFieldFrameInterceptor(259, 2, 1, 2, 0))
	//解析HTLV+CRC数据
	s.AddInterceptor(&interceptor.HtlvcrcInterceptor{})
	//开启服务
	s.Serve()
}
