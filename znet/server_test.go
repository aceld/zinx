...
func ClientTest(i uint32) {
	...
	for {
		dp := NewDataPack()
		msg, _ := dp.Pack(NewMsgPackage(uint8(i), uint8(i), []byte("client test message")))
...
func (this *PingRouter) Handle(request ziface.IRequest) {
	...
	err := request.GetConnection().SendMsg(1, 1, []byte("ping...ping...ping\n"))
	if err != nil {
...
func (this *HelloRouter) Handle(request ziface.IRequest) {
	...
	err := request.GetConnection().SendMsg(2, 2, []byte("hello zix hello Router"))
	if err != nil {
		fmt.Println(err)
	}
}
...
func DoConnectionBegin(conn ziface.IConnection) {
	...
	err := conn.SendMsg(2, 2, []byte("DoConnection BEGIN..."))
	if err != nil {
		fmt.Println(err)
	}
}
